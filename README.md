...
文化祭スタッフのシフト管理を行うための PWA（Progressive Web App）アプリケーションである。リアルタイム同期、プッシュ通知、オフライン対応、メンテナンスモード、団体付与シフト機能、チャット機能、そしてスムーズな UI アニメーションを備えている。
...
- **通知許可**:
  ...
  - 許可すると、端末ごとの Push トークンが `push_subscriptions` テーブルに登録される。
